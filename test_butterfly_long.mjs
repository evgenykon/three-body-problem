...
engine.addBody({ id: 'b1', position: { x: -1, y: 0 }, velocity: { x: 0.30689, y: 0.12551 }, mass: 1, radius: 0.005, color: '#ff6b6b' });
engine.addBody({ id: 'b2', position: { x: 1, y: 0 }, velocity: { x: 0.30689, y: 0.12551 }, mass: 1, radius: 0.005, color: '#4ecdc4' });
engine.addBody({ id: 'b3', position: { x: 0, y: 0 }, velocity: { x: -0.61378, y: -0.25102 }, mass: 1, radius: 0.005, color: '#45b7d1' });
let minDist = Infinity, energy0 = 0;
for (let f = 0; f < 2000; f++) {
  ...
  if (d12 < 0.01 || d13 < 0.01 || d23 < 0.01) { console.log('COLLISION at frame', f); process.exit(0); }
}
const drift = ((engine.totalEnergy() - energy0) / energy0 * 1e6).toFixed(3);
console.log('OK 2000 frames | minDist:', minDist.toFixed(6), '| energy drift:', drift, 'ppm');
